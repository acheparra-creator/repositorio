...
## Project-Specific Principles — Claude Ads

### 9. Free-First by Default

**Description**: Every data integration must offer a zero-cost path before requiring a paid SaaS or partner-API approval.

**Rationale**: Claude Ads targets freelancers and small teams. Paid dependencies as hard requirements exclude the primary audience and undermine the tool's value proposition.

**Application**:
- Implement three data tiers in strict order: MCP → Direct API (stdlib, OAuth) → Manual export CSV.
- Document paid alternatives (Adspirer, Zernio) as optional Tier 3 only.
- All `scripts/api/` adapters must be pure stdlib (Python 3.10+, no external packages).
- Never make a paid integration a required code path.

### 10. JSON Contracts are Non-Negotiable

**Description**: Every `audit-*` agent must emit both a human-readable Markdown file and a machine-parseable JSON file validated against `audit-output-schema.json`.

**Rationale**: The orchestrator aggregates scores programmatically. Markdown-only output breaks the cross-platform scoring pipeline silently — no error is raised, the user just gets wrong totals.

**Application**:
- Each agent's SKILL.md must declare both output files (`<platform>-audit-results.md` and `<platform>-audit-results.json`).
- CI validates JSON structure in every PR via `pytest tests/`.
- Any new agent type that feeds into aggregate scoring must adopt the same contract.

### 11. Skill Token Budget

**Description**: SKILL.md files must stay under 500 lines / 5000 tokens. Reference files must stay under 200 lines.

**Rationale**: Claude Code skill files are loaded into the model context on each invocation. Bloated skills consume context budget that should be available for audit data and analysis.

**Application**:
- CI enforces the 200-line reference cap (`find ads/references -name "*.md" | xargs wc -l`).
- When a skill grows past the limit, extract context into `ads/references/` on-demand files.
- Orchestrator skills route to sub-skills; they must not restate sub-skill logic inline.

### 12. Profile-First User State

**Description**: Every user-facing command must read `~/.claude-ads/profile.json` before asking inline questions.

**Rationale**: Repeating onboarding questions on every invocation degrades UX. The profile captures OAuth setup status, industry, and preferences so the skill can skip what it already knows.

**Application**:
- Run `python3 scripts/profile.py get` first; on exit 2 (no profile), surface a one-line tip to run `/ads start` but allow inline fallback.
- Never store API keys or tokens in the profile — only `*_present: true/false` flags.
- Use `scripts/profile.py set` for all writes so validation against `profile-schema.json` stays consistent.